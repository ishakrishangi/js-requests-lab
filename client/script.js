...
const repeatMyParam = () => { 
    //YOUR CODE HERE
  axios.get(`http://localhost:3000/repeat/i-am-stressed`)
    .then(res => {
        console.log(res.data)
   let repeatIt= document.getElementById('repeat-text')
   repeatIt.textContent = res.data
   repeatIt.style.display = 'block';
    })
}
...
const queryTest = () => {
    axios.get(`http://localhost:3000/query-test?myquery=a-really-awesome-query`)
    .then(res => console.log(res.data))
}
